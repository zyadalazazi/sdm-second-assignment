...
public class Exercise_3 {
    ...
    // A hashmap to keep track of the shortest path by recording <sourceId, parentId>
    private static HashMap<Long, Long> pathMap;
    ...
    private static class sendMsg extends AbstractFunction1<EdgeTriplet<Integer,Integer>, Iterator<Tuple2<Object,Integer>>> implements Serializable {
        ...
        public Iterator<Tuple2<Object, Integer>> apply(EdgeTriplet<Integer, Integer> triplet) {
            ...
            Integer pathLength = sourceVertex._2 + edge;

            if (sourceVertex._2 == Integer.MAX_VALUE){
                return JavaConverters.asScalaIteratorConverter(new ArrayList<Tuple2<Object, Integer>>().iterator()).asScala();
            }
            else if (pathLength < dstVertex._2) {   // source vertex value is smaller than dst vertex?
                pathMap.put(triplet.dstId(), triplet.srcId());
                // propagate source vertex value
                return JavaConverters.asScalaIteratorConverter(Arrays.asList(
                        new Tuple2<Object, Integer>(triplet.dstId(), pathLength)).iterator()).asScala();
            } else {
                // do nothing
                return JavaConverters.asScalaIteratorConverter(new ArrayList<Tuple2<Object, Integer>>().iterator()).asScala();
            }
    ...
    public static void shortestPathsExt(JavaSparkContext ctx) {
        ...
        ops.pregel(Integer.MAX_VALUE,
                ...
                .foreach(v -> {
                    ...
                    String pretext = "Minimum cost to get from " + labels.get(1l) + " to "+labels.get(vertex._1)+" is [";

                    /*
                     * Here we simply use the idea of tracing back through `pathMap` from destination to source
                     * as discussed in theory class
                     */
                    Long currentId = (Long) vertex._1;

                    if(pathMap.get(currentId) == null) { // We are at the source which has no parent
                        sb.append(labels.get(currentId)).append("] ");
                    } else {
                        // Add the source itself
                        sb.append(labels.get(currentId));
                        // Traceback to the parent which has no parent indicated by a null
                        do {
                            Long parentId = pathMap.get(currentId);
                            sb.append(",").append(labels.get(parentId));
                            currentId = parentId;
                        } while(pathMap.get(currentId) != null);
                        // Since we were tracing back to source, we need to print in reverse order
                        sb.reverse();
                        sb.append("] ");
                    }
                    System.out.println(pretext + sb + "with cost " + vertex._2);
                });
